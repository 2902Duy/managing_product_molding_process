    ...
    <section className="mb-8 relative z-20">
      <div className="bg-notion-white border border-whisper rounded-[8px] shadow-card">
              ...
              {outputs.map(item => (
                <tr key={item.id} className="group hover:bg-warm-white/40 transition">
                  <td className="p-0 border-r border-whisper relative">
                    <select
                      className="w-full h-full min-h-[36px] bg-transparent px-3 py-2 outline-none hover:bg-black/[0.02] focus:bg-white focus:ring-1 focus:ring-inset focus:ring-notion-blue transition-colors text-[13px] text-notion-black appearance-none cursor-pointer"
                      value={item.name}
                  ...
                  <td className="p-0 border-r border-whisper">
                    <input type="number" placeholder="0" className={numInputClass} value={item.thickness} onChange={e => onChangeOutput(item.id, 'thickness', e.target.value)} />
                  </td>
                  <td className="p-0 border-r border-whisper">
                    <input type="number" placeholder="0" className={numInputClass} value={item.width} onChange={e => onChangeOutput(item.id, 'width', e.target.value)} />
                  </td>
                  <td className="p-0 border-r border-whisper">
                    <input type="number" placeholder="0" className={numInputClass} value={item.length} onChange={e => onChangeOutput(item.id, 'length', e.target.value)} />
                  </td>
                  <td className="p-0 border-r border-whisper bg-green-50/10">
                    <input type="number" placeholder="0" className={`${numInputClass} font-semibold text-success-green`} value={item.quantity} onChange={e => onChangeOutput(item.id, 'quantity', e.target.value)} />
                  </td>
                  <td className="p-0 border-r border-whisper">
                    <input type="number" step="0.0001" placeholder="Tự tính" className={`${numInputClass} bg-[#fcfcfb] focus:bg-white`} value={item.volume} onChange={e => onChangeOutput(item.id, 'volume', e.target.value)} />
                  </td>
                  <td className="p-0 border-r border-whisper relative">
                    <select
                      className="w-full h-full min-h-[36px] bg-transparent px-3 py-2 outline-none hover:bg-black/[0.02] focus:bg-white focus:ring-1 focus:ring-inset focus:ring-notion-blue transition-colors text-[13px] text-notion-black appearance-none cursor-pointer font-medium"
                      value={item.status}
                  ...
                  <td className="p-0 text-center">
                    <button onClick={() => onRemoveOutput(item.id)} className="text-warm-gray-300 hover:text-red-500 transition p-2"><Trash2 size={15} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="p-2 border-t border-whisper bg-notion-white rounded-b-[8px]">
          <button onClick={onAddOutput} className="flex items-center gap-1.5 text-[13px] font-medium text-notion-blue hover:text-notion-blue-hover transition px-3 py-1.5 rounded-[4px] hover:bg-notion-blue/5">
